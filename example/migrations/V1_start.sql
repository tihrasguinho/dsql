...
CREATE TABLE IF NOT EXISTS tb_likes (
  ...
  user_id UUID NOT NULL,
  created_at TIMESTAMP NOT NULL DEFAULT NOW (),
  CONSTRAINT fkpost_likes FOREIGN KEY (post_id) REFERENCES tb_posts (id) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT fkuser_likes FOREIGN KEY (user_id) REFERENCES tb_users (id) ON DELETE CASCADE ON UPDATE CASCADE
);